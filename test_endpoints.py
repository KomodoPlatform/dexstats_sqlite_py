...
if platform.node() == "markets-atomicdex-test-api":
	BASE_URL = "https://stats.testchain.xyz:8080"
else:
...
def test_authenticated_24hr_pubkey_stats():
    response = requests.get(f'{BASE_URL}/api/v1/private/24hr_pubkey_stats', auth=HTTPBasicAuth(API_USER, API_PASS))
    assert response.status_code == 200


def test_authenticated_24hr_coins_stats():
    response = requests.get(f'{BASE_URL}/api/v1/private/24hr_coins_stats', auth=HTTPBasicAuth(API_USER, API_PASS))
    assert response.status_code == 200


def test_authenticated_24hr_version_stats():
    response = requests.get(f'{BASE_URL}/api/v1/private/24hr_version_stats', auth=HTTPBasicAuth(API_USER, API_PASS))
    assert response.status_code == 200


def test_authenticated_24hr_gui_stats():
    response = requests.get(f'{BASE_URL}/api/v1/private/24hr_gui_stats', auth=HTTPBasicAuth(API_USER, API_PASS))
    assert response.status_code == 200


def test_authenticated_24hr_failed_pubkey_stats():
    response = requests.get(f'{BASE_URL}/api/v1/private/24hr_failed_pubkey_stats', auth=HTTPBasicAuth(API_USER, API_PASS))
    assert response.status_code == 200


def test_authenticated_24hr_failed_coins_stats():
    response = requests.get(f'{BASE_URL}/api/v1/private/24hr_failed_coins_stats', auth=HTTPBasicAuth(API_USER, API_PASS))
    assert response.status_code == 200
...
def test_authenticated_24hr_failed_gui_stats():
    response = requests.get(f'{BASE_URL}/api/v1/private/24hr_failed_gui_stats', auth=HTTPBasicAuth(API_USER, API_PASS))
    assert response.status_code == 200
